...
enum {
  MACRO_QWERTY,
  MACRO_VERSION_INFO,
  MACRO_MOVE_TO_COLEMAK_EN,
  MACRO_MOVE_TO_QWERTY_RU,
...
const macro_t *macroAction(uint8_t macro_id, KeyEvent &event) {
  if (keyToggledOn(event.state)) {
    ...
    case MACRO_VERSION_INFO:
      ...
      break;
    case MACRO_MOVE_TO_COLEMAK_EN:
      return MACRO(D(LeftAlt), D(LeftShift), Tc(1), U(LeftAlt), U(LeftShift),
                   Tr(MoveToLayer(0)));
      break;
    case MACRO_MOVE_TO_QWERTY_RU:
      return MACRO(Tr(MoveToLayer(1)),
                   D(LeftAlt), D(LeftShift), Tc(2), U(LeftAlt), U(LeftShift));
      break;
    case MACRO_TOGGLE_ONESHOT:
      // doesn't work
      //return MACRO(Tr(OSM(LeftShift)));
...
//  OneShot.setTimeout(400);
//  OneShot.setHoldTimeout(200);
